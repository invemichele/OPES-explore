...
# compulsory
parser.add_argument('--last-file',dest='lastfile',type=str,required=True,help='the file where the last bias is, stored as a FES')
...
print('  last bias reweighing')
lastfile = args.lastfile
biasfile = None
if args.lastfile == 'auto' and stride!=0:
  print('  auto mode')
  biasfile = 'fes_running/fes_%d.dat'
  lastfile = biasfile%1
bias_grid, bias_val = np.loadtxt(lastfile, usecols=(0,1), unpack=True)
gamma=20
bias_val = -1/(1-1/gamma) * bias_val
from scipy.interpolate import interp1d
get_bias=interp1d(bias_grid,bias_val-max(bias_val),kind='cubic',fill_value='extrapolate')
assert not dim2, "only 1D is supported with last bias scheme"
bias+=get_bias(cv_x)
##########
...
# print to file
  if stride==len_tot:
    ...
    it+=1
    if biasfile is not None and n<len_tot:
      bias_val=pd.read_table(biasfile%it,dtype=float,sep='\s+',comment='#',header=None,usecols=[1])
      bias_val = -1/(1-1/gamma)*np.array(bias_val).reshape(-1)/kbt
      get_bias=interp1d(bias_grid,bias_val-max(bias_val),kind='cubic',fill_value='extrapolate')
      bias=get_bias(cv_x)
